...
void SDLUI_Render_Button(SDLUI_Control_Button *btn)
{
	if(btn->visible)
	{
		if(btn->text.modified)
		{
			...
			SDL_Surface* s = btn->texturePath ? IMG_Load(btn->text.data) : TTF_RenderText_Blended(SDLUI_Font.handle, btn->text.data, SDLUI_Core.theme.col_white);
...
void SDLUI_CONTROL_ITEM(SDLUI_Control_Item* btn)
{
	if (btn->visible)
	{
		if (btn->text.modified)
