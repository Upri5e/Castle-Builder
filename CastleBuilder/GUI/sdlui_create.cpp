...
SDLUI_Control_Item* SDLUI_CreateItem(SDLUI_Control_Window *wnd, i32 x, i32 y, char const *text)
{
	SDLUI_Control_Item *btn = (SDLUI_Control_Item*)malloc(sizeof(SDLUI_Control_Item));

	btn->type = SDLUI_CONTROL_TYPE_ITEM;
...
SDLUI_Control_Button* SDLUI_CreateButton(SDLUI_Control_Window* wnd, i32 x, i32 y, char const* text)
{
	...
	btn->state = SDLUI_BUTTON_STATE_NORMAL;
	btn->parent = wnd;
	btn->texturePath = text;
	SDL_Surface* s = btn->texturePath ? IMG_Load(btn->texturePath) : TTF_RenderText_Blended(SDLUI_Font.handle, text, SDLUI_Core.theme.col_white);
	btn->tex_text = SDL_CreateTextureFromSurface(SDLUI_Core.renderer, s);
	SDL_FreeSurface(s);
